...
export async function saveOrShareBlob(
  ...
  if (Capacitor.isNativePlatform()) {
    ...
    // Modern Native Path: Storing inside user's public Documents folder
    ...
    try {
      ...
      // 2. If it's just a direct save action, exit early. Otherwise, trigger Share sheet.
      if (forceSaveDirectly) {
        return;
      }

      // Open native mobile share sheet using the file uri generated by Capacitor Filesystem
      await Share.share({
        title: fileName,
